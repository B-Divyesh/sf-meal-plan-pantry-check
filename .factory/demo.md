...
- Storage: demo state is in memory only. Demo mode does not open the real IndexedDB database or read license localStorage keys.
- Reset: choose **Reset demo** in the persistent demo banner.
- Exit: choose **Start for real**. This reloads `/` and opens the normal IndexedDB-backed planner without copying demo changes.
